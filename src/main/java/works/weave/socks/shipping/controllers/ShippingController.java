...
public class ShippingController {
    ...
    public
    @ResponseBody
    Shipment postShipping(@RequestBody Shipment shipment, @RequestHeader HttpHeaders httpHeaders) {
        System.out.println("Adding shipment to queue...");
        TraceUtil.instance().newProcess(() -> {
            try {
                rabbitTemplate.convertAndSend("shipping-task", shipment, message -> {
                    message.getMessageProperties().getHeaders().putAll((Map<String, String>) TraceUtil.instance().serializeContext());
                    return message;
                });
            } catch (Exception e) {
        ...
        }, "Post Shipping", TraceUtil.instance().deserializeContext((Serializable) httpHeaders.toSingleValueMap()));
        return shipment;
    }
